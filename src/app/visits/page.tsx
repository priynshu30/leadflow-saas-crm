...
export default function FieldVisitsPage() {
  ...
  const fetchVisits = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/visits?filter=${filterTab}`);
      if (res.ok) {
      ...
      <div className="space-y-6 max-w-6xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
              <MapPin className="h-6 w-6 text-indigo-600" /> Field Visits & Client Meetings
            </h1>
            <p className="text-sm text-slate-500 mt-0.5">
              Track on-site client visits, GPS check-ins, site photos, meeting durations & closed deals
            </p>
          </div>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-2xl border border-dashed border-slate-300 p-12 text-center">
            ...
            <p className="font-bold text-slate-700">No field visits logged yet</p>
                    ...
                    <div className="space-y-2.5 flex-1">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-500 mt-0.5">
                          Conducted by: <strong>{visit.user?.name || "Agent"}</strong>
                        </p>
                      </div>
